...
These guidelines provides recommendations for defining JSON data at Unifly.
...
The guidelines clarify some specific cases to allow Unifly JSON data to have
...
[#118]
== {MUST} property names must be ASCII [yellow-background]#snake_case# (and never camelCase): `^[a-z_][a-z_0-9]*$`
...
should be [yellow-background]#represented using additionalProperties with a schema defining the
...
To indicate they contain multiple values prefer to [yellow-background]#pluralize array
...
values, true and false. [yellow-background]#If the content has a meaningful null value,
...
[yellow-background]#If a property is defined to be not `required` and `nullable` (see
...
[yellow-background]#The only exception to this rule is JSON Merge Patch {RFC-7396}[RFC 7396]) which
...
Empty array values can unambiguously be [yellow-background]#represented as the empty list, `[]`.#
...
[yellow-background]#Dates and date-time properties should end with# `[yellow-background]#_at#` to distinguish them from
boolean properties which otherwise would have very similar or even identical
names:

- [yellow-background]#{created_at} rather than {created},#
...
Use the [yellow-background]#date and time formats# defined by {RFC-3339}#section-5.6[RFC 3339]:

* for "date" use strings matching
`date-fullyear "-" date-month "-" date-mday`, for example: `[yellow-background]#2015-05-28#`
* for "date-time" use strings matching `full-date "T" full-time`, for
example `[yellow-background]#2015-05-28T14:07:17Z#`, `[yellow-background]#2015-05-28T14:07:17.12345Z#`,
or `[yellow-background]#2015-05-28T14:07:17.12Z#`
...
defined by the standards. However, we [yellow-background]#encourage restricting dates to UTC and
...
saving time. [yellow-background]#Localization of dates should be done by the services that provide
...
be strings formatted as recommended by
{ISO-8601}[ISO 8601]({RFC-3339}#appendix-A[Appendix A of RFC 3339 contains a grammar]
for durations) such as `[yellow-background]#P34Y6M2DT13H34M13S#` for duration and
`[yellow-background]#2015-05-28T14:07:17Z/2015-05-28T14:07:17.1234Z#` for a time interval.
