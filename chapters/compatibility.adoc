...
We strongly [yellow-background]#encourage using compatible API extensions and discourage versioning#
...
breaking changes when deployed into operation. However, [yellow-background]#in specific controlled
...
API designers should apply the following [yellow-background]#rules to evolve RESTful APIs for
services in a backward-compatible way:#
[yellow-background]
* Add only optional, never mandatory fields, unless the resource supports {PUT} semantics:
  an old client doing a {PUT} will inadvertendly clear the optional fields of the newer API.
...
[yellow-background]#Service clients should apply the robustness principle#:
...
[yellow-background]#Designers of service provider APIs should be conservative and accurate in what
...
[yellow-background]#Not ignoring unknown input fields is a specific deviation from Postel's Law
(e.g. see also +
...
[yellow-background]#In a response body, you must always return a JSON object (and not e.g. an
...
to easily extend your response and [yellow-background]#e.g. add pagination later, without breaking
...
The Open API 3.0 specification is not very specific on default extensibility
of objects, and redefines JSON-Schema keywords related to extensibility, like
`additionalProperties`. Following our overall compatibility guidelines, [yellow-background]#Open
...
When it comes to Open API 3.0, this means an `additionalProperties` declaration
...
these issues, we strongly recommend to [yellow-background]#use an open-ended list of values instead
...
[yellow-background]#If changing an API can’t be done in a compatible way, then proceed in one of
these three ways:#
[yellow-background]
...
we [yellow-background]#strongly recommend to only use the first two approaches#.
...
[yellow-background]#Media type versioning: Here, version information and media type are provided
together via the HTTP Content-Type header — e.g.#
`[yellow-background]#application/x.unifly.cart+json;version=2#`. For incompatible changes, a new
...
[source,http]
----
Accept: application/x.unifly.cart+json;version=2
----
...
[source,http]
----
Content-Type: application/x.unifly.cart+json;version=2
----
...
complexer release management [yellow-background]#we do not use URI versioning, and go instead with
