...
[yellow-background]#An important principle for API design and usage is Postel's
...
[yellow-background]#This certainly applies to service clients, but service provider APIs should be
conservative and accurate in what they accept. This is a deliberate deviation
from Postel's Law!#
...
As mentioned above, Unifly is transforming from an application into an
expansive UTM platform comprising of a set of products offering an API
based integration model with our business partners. As a
company we want to deliver products to our (internal and external)
customers which can be consumed like a service.

[yellow-background]#Platform products provide their functionality via (public) APIs; hence,
...
API First is one of our engineering and architecture principles. [yellow-background]#In a nutshell API First requires two
aspects:#
[yellow-background]
