...
[yellow-background]#Every API endpoint needs to be secured using OAuth 2.0.# Please refer to
the
...
components:
  securitySchemes:
    oauth2:
      ...
      flows:
        clientCredentials:
          tokenUrl: https://identity.unifly.aero/oauth2/token
          scopes:
...
be omitted. [yellow-background]#API endpoints should always set# `[yellow-background]#flow: clientCredentials#`
[yellow-background]#and ignore this information.#


[#105]
== {MUST} define and assign permissions (scopes)

[yellow-background]#APIs must define permissions to protect their resources. Thus, at least one
...
This pattern is compatible with the previous definition and results in permissions
such as `#order-management.sales_order.read#` or `#order-management.shipment_order.read#`.
