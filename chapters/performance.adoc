...
[yellow-background]#Compress the payload of your API’s responses with gzip, unless there’s a good
...
*ToDo* Investigate whether we should do this for all APIs or only APIs consumed by
...
[yellow-background]#The API should clearly document which fields are supported. If none are documented, the default
behavior is no support for partial responses at all. Typically our first versions won't support
filtering.#
...
[Rule #157](https://opensource.unifly.aero/restful-api-guidelines/#157)`
