...
[yellow-background]#{GET} requests are used to *read* either a single or a collection resource.#
...
* [yellow-background]#{GET} requests must NOT have a request body payload (see {GET-with-Body})#
...
1. [yellow-background]#{GET} with URL encoded query parameters: when it is possible to encode the
...
[yellow-background]#{PUT} requests are used to *update* (in rare cases to create) *entire*
...
* [yellow-background]#successful {PUT} requests will usually generate {200} or {204} (if the
resource was updated – preferrably with or else without actual content returned), and {201} (if
the resource was created)#
* {PUT} should only be used for resources that we expect to be stable in newer versions
  of the API or for creating immutable resources. If we decide to add (optional) fields
  to an existing API that supports {PUT}, this version is incompatible with the previous
  version, since the old clients might clear the optional fields of the newer version
  when using a {PUT}.
...
*Note:* [yellow-background]#In the rare cases where {PUT} is although used for resource creation,
...
*Hint:* [yellow-background]#To prevent unnoticed concurrent updates and duplicate creations when
...
{POST} requests are [yellow-background]#idiomatically used to *create* single resources# on a
...
* [yellow-background]#successful {POST} requests will usually generate {200} (if resources have
...
*Hint:* [yellow-background]#Posting the same resource twice is *not* required to be <<idempotent>>
...
[yellow-background]#{PATCH} requests are used to *update parts* of single resources#, i.e. where only
...
* [yellow-background]#successful {PATCH} requests will usually generate {200} or {204} (if
resources have been updated preferrably with or else without updated content returned)#
...
1. use {PUT} with complete objects to update a stable or create an immutable
   resource as long as feasible (i.e. do not use {PATCH} at all).
2. [yellow-background]#use {PATCH} with partial objects to only update parts of a resource,
...
* [yellow-background]#successful {DELETE} requests will usually generate {200} (if the deleted
resource is returned) or {204} (if no content is returned)#
* [yellow-background]#failed {DELETE} requests will usually generate {404}
TODO: check whether we're sure we don't want to return a 204 in such a case since
{DELETE} is idempotent and may be retried.#

*Important:* After deleting a resource with {DELETE}, a {GET} request on the
resource is expected to return {404} (not found). Under no
...
* [yellow-background]#{HEAD} has exactly the same semantics as {GET}, but returns headers only, no
body.#
...
[yellow-background]#{OPTIONS} requests are used to *inspect* the available operations (HTTP
...
*Note:* [yellow-background]#{OPTIONS} is rarely implemented, though it could be used to
self-describe the full functionality of a resource. TODO: do we (or better, Spring) implement it?#
...
[yellow-background]#In many cases it is helpful or even necessary to design {POST} and {PATCH}
...
[yellow-background]#If you mainly aim to support safe retries, we suggest to apply <<182,
...
[yellow-background]#The most important pattern to design {POST} <<idempotent>> for creation is to
...
*Note:* [yellow-background]#When using the secondary key pattern without {Idempotency-Key} all
...
| Header | `[yellow-background]#Header: value1,value2#` | `Header: value1, Header: value2`
| {RFC-7230}#section-3.2.2[RFC 7230 Section 3.2.2]

| Query | `[yellow-background]#?param=value1,value2#` | `?param=value1&param=value2`
...
| Header | `[yellow-background]#style: simple, explode: false#` | not allowed (see
  {RFC-7230}#section-3.2.2[RFC 7230 Section 3.2.2])
| Query  | `[yellow-background]#style: form, explode: false#`   | `style: form, explode: true`
|===============================================================
...
[yellow-background]#We prefer the use of query parameters to describe resource-specific
...
* `[yellow-background]#name=Unifly#`, to query for elements based on property equality
* `[yellow-background]#age=5#`, to query for elements based on logical properties
**  Assuming that elements don't actually have an `age` but rather a `birthday`
* `[yellow-background]#max_length=5#`, based on upper and lower bounds (`min` and `max`)
* `[yellow-background]#shorter_than=5#`, using terminology specific e.g. to _length_
* `[yellow-background]#created_before=2019-07-17#` or `[yellow-background]#not_modified_since=2019-07-17#`
...
[yellow-background]#APIs that qualify for a specific, complex query language are encouraged to
...
[yellow-background]#Implicit filtering could be done on:#

* [yellow-background]#the collection of resources being return on a parent {GET} request#
* [yellow-background]#the fields returned for the resource's detail#

[yellow-background]#In such cases, the implicit filtering must be in the API specification (in
its description).#
