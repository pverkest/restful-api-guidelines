...
SalesOrder:
  properties:
    grand_total:
      $ref: 'https://opensource.unifly.aero/restful-api-guidelines/money-1.0.0.yaml#/Money'
----
...
* Breaks existing library support, e.g. https://github.com/unifly/jackson-datatype-money[Jackson Datatype Money]
...
[yellow-background]#There exist a variety of field types that are required in multiple places. To
...
* [yellow-background]#[[id]]{id}: the identity of the object. If used, IDs must be opaque strings
  ...
  cross entities.
  [yellow-background]#TODO: Decide whether we'll enforce the use of links (or HAL) or not.#
* [yellow-background]#[[xyz_id]]{xyz_id}: an attribute within one object holding the identifier of
  ...
  `[yellow-background]#partner_id#` not `partner_number`, or `parent_node_id` for the reference to a parent
  node from a child node, even if both have the type `Node`).
* [yellow-background]#[[created_at]]{created_at}: when the object was created.# If used, this must
...
* [yellow-background]#[[modified_at]]{modified_at}: when the object was updated.# If used, this must
  be a `date-time` construct. Originally named [[modified]]{modified} before
  adding the <<235, naming conventions for date/time properties>>.
* [yellow-background]#[[type]]{type}: the kind of thing this object is.# If used, the type of this
...
* [yellow-background]#[[etag]]{etag}: the <<182, ETag>> of an <<158, embedded sub-resource>>#. It
...
allows API client developers to build up a common understanding of Unifly's
...
[yellow-background]#TODO Define _reference, _code, and ???#
...
* [yellow-background]#[[self]]{self}#:the link or cursor in a pagination response or object
  pointing to the same collection object or page.
* [yellow-background]#[[first]]{first}#: the link or cursor in a pagination response or object
  pointing to the first collection object or page.
* [yellow-background]#[[prev]]{prev}#: the link or cursor in a pagination response or object
  pointing to the previous collection object or page.
* [yellow-background]#[[next]]{next}#: the link or cursor in a pagination response or object
  pointing to the next collection object or page.
* [yellow-background]#[[last]]{last}#: the link or cursor in a pagination response or object
...
* [yellow-background]#[[items]]{items}#: array of resources, holding all the items of the current
...
* [yellow-background]#[[query]]{query}#: object containing the query filters applied in the search
...
[yellow-background]#TODO: Check whether this is the addressee/address we want#
