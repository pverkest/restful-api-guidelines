...
Therefore, you must [yellow-background]#define **all** success and service specific error
...
**Hint:** In most cases it is [yellow-background]#not useful to document all technical errors,
  ...
  default:
    ...
    content:
      "application/problem+json":
        schema:
          $ref: 'https://opensource.unifly.aero/problem/schema.yaml#/Problem'
----
...
You must only [yellow-background]#use standardized HTTP status codes consistently with their
...
The goal is not only to be as precise as possible, but also to keep this as
simple as possible and to [yellow-background]#keep this list as short as we can#. If
a specific code (typically 4xx or 5xx) isn't present in this list, we use {400}
or {500} instead.
...
You must [yellow-background]#use the most specific HTTP status code from <<150>>#
when returning information about your request processing status or error situations.
...
* [yellow-background]#A batch or bulk request *always* has to respond with HTTP status code
...
[yellow-background]#APIs that wish to manage the request rate of clients must use the {429} (Too
...
[yellow-background]#{RFC-7807}[RFC 7807] defines a Problem JSON object and  the media type
...
responses:
  503:
    ...
    content:
      "application/problem+json":
        schema:
          $ref: 'https://opensource.zalando.aero/problem/schema.yaml#/Problem'
----

[yellow-background]#**Hint for developers / implementation:** have a look at https://github.com/zalando/problem
and https://github.com/zalando/problem-spring-web!#
...
and on which clients should never rely. Moreover, [yellow-background]#stack traces can leak
